...
export function ContactContent({
  ...
  // Debug logging
  console.log("ContactContent Debug:");
  console.log("map prop:", map);
  console.log("contactProps:", contactProps);
  console.log("selectedOffice:", selectedOffice);
  console.log("selectedOffice?.mapUrl:", selectedOffice?.mapUrl);

  // Generate proper map URL if needed
  const getMapUrl = (office: any) => {
    if (!office) return null;
    
    // If mapUrl exists and is a proper embed URL, use it
    if (office.mapUrl?.includes('maps/embed')) {
      return office.mapUrl as string;
    }
    
    // If we have address information, generate a Google Maps embed URL (without API key)
    if (office.address) {
      const address = `${office.address.street}, ${office.address.city}, ${office.address.state}, ${office.address.country}`;
      const encodedAddress = encodeURIComponent(address);
      return `https://www.google.com/maps?q=${encodedAddress}&output=embed`;
    }
    
    // Fallback: use a default location (León, Spain) without API key
    return "https://www.google.com/maps?q=León,Spain&output=embed";
  };

  const mapUrl = getMapUrl(selectedOffice);
          ...
          <CardFooter>
            <div className="w-full">
              {mapUrl ? (
                ...
                <div className="flex h-[150px] items-center justify-center rounded-md border border-dashed border-gray-300 bg-gray-50">
                  <p className="text-sm text-muted-foreground">
                    Mapa no disponible - Original mapUrl: {selectedOffice?.mapUrl || "undefined"}
                  </p>
                </div>
              )}
            </div>
