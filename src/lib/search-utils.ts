...
export function buildSearchSlug(params: SearchParams): string {
  ...
  if (params.propertyType && params.propertyType !== "any" && typeToSlug[params.propertyType]) {
    typeSegment += typeToSlug[params.propertyType];
  } else {
  ...
  // Add location
  if (params.location) {
    const locationSlug = normalizeForUrl(params.location);
    segments.push(`en-${locationSlug}`);
  } else {
...
export function parseSearchSlug(slug: string): SearchParams {
  ...
  if (segments.length > 0) {
    ...
    const slugToType: Record<string, PropertyType> = {
      "-casas": "casa",
      "-pisos": "piso",
      "-locales": "local",
      "-solares": "solar",
    ...
    for (const [slug, type] of Object.entries(slugToType)) {
      if (typeSegment.includes(slug)) {
  ...
  if (segments.length > 1 && segments[1] !== "todas-ubicaciones") {
    ...
    if (locationSegment === "oportunidad") {
      ...
      params.location = decodeURIComponent(locationSegment.replace(/-/g, " "));
    }
  }
